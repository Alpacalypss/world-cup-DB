...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' UNION SELECT name FROM teams INNER JOIN games ON teams.team_id = games.opponent_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY name ASC;")"
